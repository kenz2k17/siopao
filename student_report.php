...
if($grade_id == 'all'){
	$grade_label = 'All Students List';
}else{
	$row = $system->selected_grade($grade_id);
	$grade_label = $row['grade'].' List';
}
...
/* 	public function Header() {
	...
	public function Header() {	
		...
		// Title
		$this->Cell(120, 0, 'EB Magalona National High School Student List', 0, false, 'C', 0, '', 0, false); 
		$this->Ln();
		$this->Cell(0, 0, 'EB Magalona Negros Occidental', 0, false, 'C', 0, '', 0, false); 
		$this->Ln();  
		$this->Cell(0, 0, 'School Year '.$sy.'', 0, false, 'C', 0, '', 0, false); 
		$this->Ln();
		$this->Cell(0, 0, ''.$grade_label.'', 0, false, 'C', 0, '', 0, false); 		
